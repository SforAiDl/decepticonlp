...
loss_type_list = [torch.nn.modules.loss.BCELoss, 
...
class CharAttackerWithHuggingFace():
	...
	def __init__(self, model, data_loader, adversarial_data_loader, input_format, accuracy=True, logs_after_every=50, device=torch.device('cpu')):
	...
	def attack(self):
		...
		self.loss_logs['original'], self.accuracy_logs['original'] = self.inference_loop(self.data_loader)
		
		print("Running inference on adversarial dataset")
		self.loss_logs['adversarial'], self.accuracy_logs['adversarial'] = self.inference_loop(self.adversarial_data_loader)
...
class CharAttacker():
	...
	def __init__(self, model, data_loader, adversarial_data_loader, criterion, input_format, apply_activation=False, threshold=0.5, logs_after_every=50, device=torch.device('cpu')):
		#Assertion tests
	...
	def inference_loop(self, dataloader):
		...
		for batch in tqdm_dataloader:
			...
			if(type(self.criterion['loss']) == torch.nn.modules.loss.BCELoss):
			...
			elif(type(self.criterion['loss']) == torch.nn.modules.loss.BCEWithLogitsLoss):
				...
				computed_loss = self.criterion['loss'](outputs.float().view(labels.shape), labels.float())
				outputs = torch.sigmoid(outputs)
			...
			if(self.criterion['accuracy']):
				...
				if((type(self.criterion['loss']) == torch.nn.modules.loss.BCELoss) or (type(self.criterion['loss']) == torch.nn.modules.loss.BCEWithLogitsLoss)):
					computed_labels = (outputs>self.threshold).float()
				
				elif((type(self.criterion['loss']) == torch.nn.modules.loss.NLLLoss) or (type(self.criterion['loss']) == torch.nn.modules.loss.CrossEntropyLoss)):
